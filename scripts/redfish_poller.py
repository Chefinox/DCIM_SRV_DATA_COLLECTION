...
import os
def get_secret(name, default_val):
    try:
...
kill_switch = PollerKillSwitch("redfish_poller")
...
def redfish_get(server, path):
    ...
    if kill_switch.is_killed():
        sys.stderr.write(f"WARN: Kill switch engaged. Aborting poll for {server['ip']}.\n")
        return None
        
    url = f"https://{server['ip']}/redfish/v1{path}"
    
    # Acquire rate limit slot
    limiter = get_limiter(server['ip'], REDFISH_LIMITS)
    if not limiter.acquire(timeout_sec=TIMEOUT):
        sys.stderr.write(f"WARN: Rate limit timeout for {server['ip']}\n")
        return None
