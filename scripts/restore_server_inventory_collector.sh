#!/usr/bin/env bash
#
# add-collect-data-server.sh  (ROLLBACK)
# Mengembalikan satu entry server (berdasarkan IP) ke list REDFISH_SERVERS
# di dalam server_inventory_collector.py, menggunakan snapshot yang dibuat oleh
# remove-collect-data-server.sh, lalu (opsional) copy ke container.
#
# Penggunaan:
#   ./add-collect-data-server.sh <IP>
#
