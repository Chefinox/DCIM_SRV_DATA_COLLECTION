...
def resolve_device_type(raw_message, source_topic):
    # 1. First priority: explicit tag from Telegraf
    tags = raw_message.get("tags", {})
    if tags.get("device_type"):
        return tags["device_type"]
    
    # 2. Second priority: infer from source topic name (using config mapping)
    ...
    # 3. Third priority: infer from measurement name
...
def resolve_metric(raw_message):
    ...
    # Try to extract the primary metric value
...
def process_message(raw_message, source_topic):
    ...
    # Device identity - prioritize tags.hostname
    ...
    # Create event_time (ISO-8601 UTC)
    ...
    # Clean up tags.host (Telegraf internal) from raw_tags
