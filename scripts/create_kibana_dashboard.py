...
def save_object(obj_type: str, obj_id: str, attributes: dict) -> str:
    ...
    resp = requests.post(
        ...
        headers=HEADERS,
        json=payload,
        auth=("elastic", "ES_OLD_PASSWORD_REDACTED")
    )
...
def create_dashboard(panels: dict):
    ...
    resp = requests.post(f"{KIBANA_URL}/api/saved_objects/dashboard/dcim-main-dashboard?overwrite=true", headers=HEADERS, json={"attributes": dashboard_attrs, "references": references}, auth=("elastic", "ES_OLD_PASSWORD_REDACTED"))
...
if __name__ == "__main__":
    resp = requests.get(f"{KIBANA_URL}/api/status", headers=HEADERS, auth=("elastic", "ES_OLD_PASSWORD_REDACTED"))
    if resp.status_code != 200:
        print(f"❌ Cannot connect to Kibana: {resp.status_code}")
        sys.exit(1)
    
    # 1. Pastikan Index Pattern Ada
    requests.post(f"{KIBANA_URL}/api/saved_objects/index-pattern/dcim-enriched-main?overwrite=true", headers=HEADERS, json={"attributes": {"title": "dcim-metrics-unified-*", "timeFieldName": "@timestamp"}}, auth=("elastic", "ES_OLD_PASSWORD_REDACTED"))
