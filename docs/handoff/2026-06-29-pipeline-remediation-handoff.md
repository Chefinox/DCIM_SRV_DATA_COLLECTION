...
curl -sk -u 'elastic:ES_OLD_PASSWORD_REDACTED' \
...
> **Catatan kredensial:** PostgreSQL user `sot_admin`; ES `elastic` / `ES_OLD_PASSWORD_REDACTED`. Kafka admin via INTERNAL listener `kafka1:29092` dari dalam container `kafka1` (binari di `/opt/kafka/bin/*.sh`). **Jangan** commit kredensial; tangani dengan hati-hati.
