...
| 2 | Kafka Broker & Topics | 75% | Topic naming per-source-type vs spec single-topic; belum ada topic `dcim.cmdb.updates`/`dcim.asset.updates` terdedikasi |
| 3 | **Validation & Normalizer** | **55% — P1 CRITICAL** | Tidak ada range validation, format/regex validation (IP/MAC/UUID), duplicate detection (Redis window), freshness/stale-timestamp check, Prometheus reject-metrics |
| 4 | Enrichment & CMDB Lookup | 80% | Impact Scoring Engine (criticality × severity) belum ada; connector ServiceNow/Jira belum ada |
| 5 | Persistence, Lineage, DLQ & Monitoring | 62% | Data Quality Scorecard 6-dimensi ke Prometheus belum ada; arsip DLQ/raw payload ke S3/MinIO belum ada |
