...
> **Menggantikan**: `docs/architecture/v4.6-pipeline-architecture.md` (v4.6.2, 2026-08-04)

> **Changelog v4.6.2 → v4.7.0 (2026-08-11, Imam Syauqi Achmad):**
> - **Granular DLQ & Lineage Architecture**: Menerapkan arsitektur DLQ 3-topik (`parse-failure`, `enrichment-failure`, `delivery-failure`) dan sistem pelacakan lineage 4-stage (`ingested`, `validated`, `enriched`, `routed`) yang selaras dengan DCIM-Wiki (PR E).
> - **Validation & Data Quality Engine**: Menambahkan modul validasi generik dengan pemeriksaan rentang (range check), validasi format (regex IP/MAC), deteksi duplikasi (berbasis window), dan pemeriksaan kesegaran data (freshness check).
> - **Impact Scoring & Data Quality Scorecard**: Diintegrasikan pola kalkulasi *impact score* (criticality × severity) dan scorecard kualitas data 6-dimensi ke pipeline.
> - **Kafka & Schema Validation**: Topik Kafka disesuaikan dan dikonfigurasi untuk menangani validasi Avro dengan lebih aman.
> - **Fixture Replay Connector**: Penambahan adapter sintetis untuk NAS dan CCTV (ISAPI) untuk testing dan referensi arsitektur.

> **Changelog v4.6.1 → v4.6.2 (2026-08-04, Imam Syauqi Achmad):**
> - **Kafka 4.1.2 Upgrade (Coordinated Cutover)**: Kafka broker di-upgrade dari 3.7.0 ke **4.1.2** (3-node co-located cluster `kafka1`, `kafka2`, `kafka3`) via coordinated simultaneous cutover untuk mencegah error `INCONSISTENT_CLUSTER_ID` akibat version skew antar voter KRaft.
> - **Kafka Log Persistence Fix**: Penambahan variabel `KAFKA_LOG_DIRS: /var/lib/kafka/data` dan Named Volumes (`kafka1_data`, `kafka2_data`, `kafka3_data`) di `docker-compose-cluster.yml` untuk menjamin persistensi log broker di disk host persisten.
> - **Multi-Host Kafka Migration Postponed**: Status migrasi multi-host dicatat sebagai **DITUNDA** (referensi teknis: KRaft static quorum tidak mendukung rolling upgrade 1-broker-per-waktu tanpa dynamic quorum / downtime terkoordinasi).
> - **MariaDB & PostgreSQL Index Optimization**:
>   - MariaDB `itop-db`: Composite index `priv_user(login, status)` untuk sub-millisecond REST API authentication.
>   - PostgreSQL `dcim_sot`: B-Tree functional index `idx_dcim_events_hostname_lower_event_time` pada `(LOWER(hostname), event_time DESC)`.
> - **iTop Consumer Code Optimizations**:
>   - Query `ILIKE` digantikan dengan `LOWER()` pada `itop_sync_utils.py` (commit `bc9bd07`).
>   - Prioritas pencarian CI class pada `find_device()` disesuaikan dengan volume terbanyak (`Server` → `NetworkDevice` → `PowerDevice` → `StorageDevice` → `Rack`) (commit `139e663`). Consumer lag tuntas dari ~3.6M ke **0**.
>
> **Changelog v4.6.0 → v4.6.1 (2026-07-27):**
> - **Kafka Topic Partitions Alignment**: Seluruh 15 topik Kafka diubah dari 1 → **12 partisi** per topik, selaras dengan spesifikasi DCIM-Wiki (`concepts/kafka-topic-design.md` §3.1). Memungkinkan paralelisme hingga 12 consumer worker per consumer group.
> - **Kafbat UI Restored**: Service `kafbat-ui` (container `dcim-kafka-ui`) yang terhapus oleh `docker system prune` di-compose ulang dan diverifikasi terhubung ke cluster `DCIM-Production` (3 broker, 18 topik, status ONLINE).
>
> **Changelog v4.5.2 → v4.6.0 (2026-07-24):**
> - **Circuit Breaker Pattern (P3 Gap Closed)**: Implementasi shared module `src/utils/circuit_breaker.py` (3-state: CLOSED, OPEN, HALF_OPEN). Diintegrasikan pada `dcim_itop_unified_consumer.py` (iTop API & Redis) dan `kafka_to_es_sync.py` (ES indexing). Menyediakan auto-routing ke DLQ saat OPEN + notifikasi grup Telegram.
> - **Data Classification Matrix (P3 Gap Closed)**: Standar resmi 4 level klasifikasi (`INTERNAL`, `CONFIDENTIAL`, `RESTRICTED`, `SECRET`) dipublikasikan di `docs/standar_dcim/data_classification_matrix.md`. Semua 52 komponen pipeline dipetakan beserta panduan teknis transit/at-rest encryption & access SLAs.
> - **Circuit Breaker Prometheus Exporter**: Ditambahkan `scripts/circuit_breaker_monitor.py` untuk meng-export status state machine ke `/tmp/dcim_circuit_breaker.prom` untuk Prometheus.
>
...
### 3.1 Kafka Cluster (3-Node, SSL/TLS, Co-located)

| Item | Detail |
|---|---|
| **Image** | `apache/kafka:4.1.2` (Upgrade dari `3.7.0` via Coordinated Cutover) |
| **Compose** | `kafka/docker-compose-cluster.yml` |
| **Nodes** | 3 broker/controller (KRaft mode, tanpa ZooKeeper: `kafka1`, `kafka2`, `kafka3`) |
...
| **Min ISR** | `2` (`min.insync.replicas=2` pada seluruh topik `dcim.*`) |
| **Storage Persistence** | `KAFKA_LOG_DIRS: /var/lib/kafka/data` (Named Volumes: `kafka1_data`, `kafka2_data`, `kafka3_data`) |
| **SSL Certs** | `kafka/certs/` (JKS keystore & truststore, CA cert PEM) |
| **Status Migrasi Multi-Host** | **DITUNDA** (KRaft static quorum tidak mendukung rolling upgrade/migrasi host 1-per-satu tanpa version skew / dynamic quorum) |

**Perubahan dari v4.6.1**: Kafka di-upgrade dari 3.7.0 ke 4.1.2. Untuk menghindari error `INCONSISTENT_CLUSTER_ID` (errorCode 104) yang dipicu oleh version skew antar voter KRaft statis, upgrade dieksekusi dengan metode *Coordinated Simultaneous Cutover* (`docker stop kafka1 kafka2 kafka3` disusul restart bersamaan). Selain itu, konfigurasi `KAFKA_LOG_DIRS` dipasang eksplisit mengarah ke Named Volume (`kafka1_data`, `kafka2_data`, `kafka3_data`) untuk menjamin daya tahan data di host.
...
**Sistem**: Apache Kafka 3-node cluster di `10.70.0.56:9092/9094` (KRaft mode, SSL/TLS)

| Kategori | Topik Kafka | Sumber Data | Format | Partitions | Replication |
|---|---|---|---|---|---|
| **Raw** | `dcim.raw.hardware.server` | NiFi → `redfish_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.hardware.server.inventory` | NiFi → `server_inventory_collector.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.power.ups` | NiFi → `snmp_ups_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.storage.nas` | NiFi → `nas_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.network.snmp` | NiFi → `mikrotik_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.network.interfaces` | NiFi → `mikrotik_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.device.isapi` | NiFi → `cctv_poller.py` | JSON | 12 | 3 |
| **Raw** | `dcim.raw.remote.ssh` | Reserved | JSON | 12 | 3 |
| **Normalized** | `dcim.normalized.events` | `dcim-normalizer.service` | **Avro** | 12 | 3 |
| **Enriched** | `dcim.enriched.events` | NiFi Enrichment Group | **Avro** | 12 | 3 |
| **Analytics** | `dcim.analytics.metrics` | `dcim-analytics-bridge` | JSON | 12 | 3 |
| **SIEM** | `dcim.siem.alerts` | NiFi → SIEM Ingestion | JSON | 12 | 3 |
| **DLQ** | `dcim.dlq.parse-failure` | `dcim-normalizer.service` | Raw bytes | 12 | 3 |
| **DLQ** | `dcim.dlq.enrichment-failure` | NiFi | Raw bytes | 12 | 3 |
| **DLQ** | `dcim.dlq.delivery-failure` | `dcim-sql-consumer`, `dcim-itop-unified` | Raw bytes | 12 | 3 |
...
## 12. L10 — Dead Letter Queue (DLQ) & Resilience (Circuit Breaker)

**Service**: `dcim-dlq-consumer.service`  
**Skrip**: `scripts/dcim_dlq_consumer.py`  
**Circuit Breaker Module**: `src/utils/circuit_breaker.py` (⚡ Baru v4.6.0)
...
| `dcim.dlq.delivery-failure` | `dcim-sql-consumer.service`, `dcim-itop-unified.service`, `kafka_to_es_sync.py` | Gagal menyimpan ke PostgreSQL/iTop/Elasticsearch atau Circuit Breaker OPEN |

### 12.1 Circuit Breaker Pattern (⚡ Baru v4.6.0)

Modul `CircuitBreaker` (`src/utils/circuit_breaker.py`) menyediakan pola ketahanan (*resilience pattern*) 3-state machine (`CLOSED`, `OPEN`, `HALF_OPEN`) untuk mencegah *cascading failures* saat dependensi eksternal *down* atau mengalami latensi tinggi:

| Protected Service | `failure_threshold` | `recovery_timeout` | `success_threshold` | Action saat OPEN |
|---|---|---|---|---|
| **iTop REST API** | 5 | 120s | 2 | Route ke `dcim.dlq.delivery-failure` + Telegram alert |
| **Redis Cache** | 3 | 30s | 2 | Fallback direct operation tanpa cache |
| **Elasticsearch** | 5 | 90s | 2 | Route ke `dcim.dlq.delivery-failure` + Telegram alert |

**Integrasi Monitoring & Alerting**:
- **Telegram Alert**: Transisi state `CLOSED -> OPEN` (trip) dan `OPEN -> CLOSED` (recovery) otomatis mengirim notifikasi ke grup Telegram (`TELEGRAM_CHAT_ID=-5266403936`).
- **Prometheus Exporter**: Skrip `scripts/circuit_breaker_monitor.py` meng-export metrik gauge `dcim_circuit_breaker_state{service="..."}` ke `/tmp/dcim_circuit_breaker.prom`.

**Lineage Tracking**: Setiap event yang masuk DLQ di-track melalui `track_lineage()` dengan `status="dlq"` atau `status="circuit_breaker_open"`.
...
### 18.5 Data Classification Matrix (⚡ Baru v4.6.0)

Dokumen standar resmi **Data Classification Matrix** dipublikasikan di `docs/standar_dcim/data_classification_matrix.md`, membagi seluruh data platform ke dalam **4 level**:

| Level | Label Klasifikasi | Contoh Data di Pipeline | Aturan Penanganan Utama |
|---|---|---|---|
| **L1** | `INTERNAL` | Metrik telemetri (CPU, Temp, Power, Fan Speed) | TLS 1.2+ transit, retensi 90 hari |
| **L2** | `CONFIDENTIAL` | Hostname, IP, Serial Number, iTop CIs, Asset Data | Wajib TLS 1.2+, RBAC, retensi 180 hari |
| **L3** | `RESTRICTED` | SIEM alerts, audit logs, `event_lineage` table, CCTV metadata | Wajib TLS 1.2+, AES-256 at-rest, retensi 365 hari |
| **L4** | `SECRET` | Vault secrets, API keys, DB passwords, SSL certs | Wajib mTLS, Vault Transit encryption, AppRole auth |
...
| Service | Skrip | Fungsi | Status v4.6.0 |
|---|---|---|---|
...
| `dcim-itop-unified.service` | `dcim_itop_unified_consumer.py` | Avro normalized → iTop (auto-create/update CI) | ✅ Aktif (**v8 + CircuitBreaker**) |
...
| *(Cron / CLI Exporter)* | `circuit_breaker_monitor.py` | Export CB metrics → `/tmp/dcim_circuit_breaker.prom` | ✅ Aktif (**⚡ Baru v4.6.0**) |

### Systemd Timers (Terjadwal)

| Timer | Service | Jadwal | Status v4.6.0 |
|---|---|---|---|
...
| `dcim-telegram-alerter.timer` | `dcim-telegram-alerter.service` | Setiap **5 menit** | ⚠️ **DEAD** (timer tidak men-trigger; service dipanggil oleh CircuitBreaker module) |

### Systemd — Dinonaktifkan (v4.6.0)
...
### NiFi Process Groups (Data Ingestion) — Live v4.6.0
...
*Dokumen ini diperbarui (v4.6.0, 2026-07-24) berdasarkan verifikasi langsung terhadap:*
- *Circuit Breaker shared module: `src/utils/circuit_breaker.py` & `tests/test_circuit_breaker.py` (6 unit tests PASS)*
- *Data Classification Matrix: `docs/standar_dcim/data_classification_matrix.md` (4-level matrix & component map)*
- *Prometheus CB exporter: `scripts/circuit_breaker_monitor.py` → `/tmp/dcim_circuit_breaker.prom`*
- *Live NiFi flow: Docker volume `nifi_conf` `flow.json.gz` — 7 process groups, 100% NiFi ingestion*
- *Source code aktif: `scripts/dcim_itop_unified_consumer.py` (iTop & Redis CB), `scripts/kafka_to_es_sync.py` (ES CB)*
- *Systemd & Exporters: 12 services active, 5 Prometheus exporters running, ES 9.3.1 + Kibana 9.3.1*
