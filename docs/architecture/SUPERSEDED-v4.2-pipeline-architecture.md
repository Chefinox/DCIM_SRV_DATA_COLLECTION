...
| **L13** | AI Training Data Archive | Arsip Elasticsearch → PostgreSQL (histori time-series jangka panjang) untuk training | ✅ Active |
...
## 15. L13 — AI Training Data Archive

> **Status**: ✅ **Active (terimplementasi 2026-06-17)**. Terverifikasi di sistem: tabel `dcim_metrics_archive` (partisi bulanan) terisi histori (~117 jt baris s/d Jun 2026: m04 ≈ 0,2 jt, m05 ≈ 46,3 jt, m06 ≈ 71,3 jt); script `scripts/es_to_pg_archive.py` aktif; timer `dcim-metrics-archive.timer` berjalan harian **03:00 WIB**; 6 materialized view `v_train_*` tersedia & berisi data. Bagian di bawah mendokumentasikan rancangan sekaligus keadaan aktual.
...
### 15.5 Komponen (status implementasi — semua sudah terpasang)
...
| `dcim-metrics-archive.timer` | `dcim-metrics-archive.service` (`scripts/es_to_pg_archive.py`) | ✅ **Aktif** — harian `03:00 WIB` (arsip inkremental ES → `dcim_metrics_archive`, L13) |
| `dcim-data-quality-check.timer` | `dcim-data-quality-check.service` | ✅ **Aktif** — harian `06:00 WIB` |

> **Catatan akses tim AI (L14)**: role read-only `dcim_ai_reader` (DDL idempoten `sql/ai_access_role.sql`) memberi SELECT pada `v_train_*`/`dcim_metrics_archive`/`dcim_failure_events` dan write hanya ke `dcim_server_anomalies`. Lihat `docs/development/ai-team-data-access-connection-guide.md`.

---

*Dokumen ini (v4.2.0, 2026-06-17) diperbarui berdasarkan verifikasi langsung terhadap konfigurasi aktif di sistem `srv-rnd-dcim` (10.70.0.56): systemd units & timers, crontab, container Docker, Kafka topics, skema & data PostgreSQL (`dcim_events`, `dcim_metrics_archive`, `v_train_*`, `dcim_server_anomalies`), role `dcim_ai_reader`, dan index Elasticsearch.*  
