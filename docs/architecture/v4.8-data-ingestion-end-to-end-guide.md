...
3. [Panduan Pengetesan Manual & End-to-End (E2E Testing Guide)](#3-panduan-pengetesan-manual--end-to-end-e2e-testing-guide)
   ...
   - [Tahap 2: Verifikasi Kafka Broker & Schema Registry (Layer 2 & 3)](#tahap-2-verifikasi-kafka-broker--schema-registry-layer-2--3)
   ...
   - [Tahap 6: Verifikasi Multi-Target Data Persistence (Layer 5)](#tahap-6-verifikasi-multi-target-data-persistence-layer-5)
...
### Tahap 2: Verifikasi Kafka Broker & Schema Registry (Layer 2 & 3)

* **Tujuan:** Memastikan pesan telemetri mengalir ke Kafka broker 12-partition dan skema Avro terdaftar di Schema Registry.
* **Perintah Uji Manual:**

  1. **Cek Ketersediaan Topik Kafka (19 Topik):**
     ```bash
     ...
     "
     ```
  2. **Cek Aliran Pesan Terbaru di Topik `dcim.raw.virtualization`:**
     ```bash
     python3 -c "
     from kafka import KafkaConsumer
     c = KafkaConsumer('dcim.raw.virtualization', bootstrap_servers='10.70.0.56:9092', auto_offset_reset='latest', consumer_timeout_ms=2000)
     for m in c:
         print('Payload:', m.value.decode('utf-8'))
     "
     ```

* **Hal yang Harus Diperhatikan & Nilai Terpenuhi:**
  - [x] Total topik Kafka aktif **19 topik** (`dcim.raw.*`, `dcim.normalized.events`, `dcim.enriched.events`, `dcim.dlq.*`).
  - [x] Seluruh topik utama memiliki **12 partisi**.
  - [x] Confluent Schema Registry (port 8081) memiliki 2 registered subjects: `dcim.enriched.events-value` dan `dcim.normalized.events-value`.
...
* **Perintah Uji Manual:**
  ...
  2. **Verifikasi Pesan Enriched di Kafka (`dcim.enriched.events`):**
     ...
     c = Consumer({'bootstrap.servers': '10.70.0.56:9092', 'group.id': 'test_inspector', 'auto.offset.reset': 'latest'})
     c.subscribe(['dcim.enriched.events'])
     msg = c.poll(3.0)
     if msg:
         print('Enriched Event Received:', msg.value()[:200])
     c.close()
     "
     ```
...
### Tahap 6: Verifikasi Multi-Target Data Persistence (Layer 5)

* **Tujuan:** Memastikan data enriched berhasil ditulis secara permanen ke PostgreSQL `dcim_sot`, TimescaleDB `dcim_analytics`, dan Elasticsearch `dcim-metrics-unified-*`.
* **Perintah Uji Manual (DB Query & ES Inspection):**

  ```bash
  python3 -c "
  import sys, psycopg2, requests, urllib3
  sys.path.append('/home/infra/dcim_metrics_project')
  from src.utils.secrets import get_secret
  urllib3.disable_warnings()

  # 1. PostgreSQL dcim_sot
  conn = psycopg2.connect(host='10.70.0.56', port=5432, dbname='dcim_sot', user='sot_admin', password='Inovasi@0918')
  cur = conn.cursor()
  cur.execute('SELECT count(*) FROM dcim_events;')
  ...
  # 2. TimescaleDB dcim_analytics
  conn2 = psycopg2.connect(host='127.0.0.1', port=5433, dbname='dcim_analytics', user='analytics_user', password='changeme')
  cur2 = conn2.cursor()
  cur2.execute('SELECT count(*) FROM metrics;')
  print('2. TSDB metrics count:', cur2.fetchone()[0])
  conn2.close()

  # 3. Elasticsearch Latest Indices
  passw = get_secret('elastic_pass', fallback_env='ELASTIC_PASSWORD')
  resp = requests.get('https://10.70.0.56:9200/_cat/indices/dcim-metrics-unified-*?v&s=index:desc', auth=('elastic', passw), verify=False)
  print('3. ES Daily Indices:\n', '\n'.join(resp.text.splitlines()[:5]))
  "
  ```

* **Hal yang Harus Diperhatikan & Nilai Terpenuhi:**
  - [x] PostgreSQL `dcim_sot.dcim_events` menampung jutaan baris data event.
  - [x] PostgreSQL `event_lineage` mencatat jejak audit 4-stage (`ingested`, `validated`, `enriched`, `routed`) $> 6.000.000$ baris.
  - [x] TimescaleDB `dcim_analytics.metrics` menampung $> 50.000.000$ baris metrik time-series.
  - [x] Elasticsearch memiliki indeks harian aktif `dcim-metrics-unified-YYYY.MM.DD` berstatus `yellow` atau `green` dengan document count bertambah secara real-time.
