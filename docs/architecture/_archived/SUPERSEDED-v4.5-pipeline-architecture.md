...
> **Menggantikan**: `docs/architecture/v4.5-pipeline-architecture.md` (v4.5.1, 2026-07-21)
...
>
> **Perubahan v4.5.1 → v4.5.2 (2026-07-24):**
> - **Kafka Broker Advertised Listeners**: Diperbaiki dari `localhost` → `10.70.0.56` agar external clients (termasuk Tim AI di `192.168.100.35`) dapat terhubung ke Kafka cluster. Commit `1f9077e`.
> - **ES + Kibana Restore**: Elasticsearch dan Kibana di-upgrade/restore ke **9.3.1**. ES/SIEM consumer koneksi diperbaiki dari SSL → HTTP (karena ES lokal tidak menggunakan TLS). Commit `baa80ea`.
> - **EnrichedEvent Schema Re-registration**: Schema `EnrichedEvent` didaftarkan ulang ke Schema Registry setelah cluster rebuild.
> - **Prometheus + Grafana Exporters**: 5 exporter Docker containers kini **aktif berjalan**: `dcim_node_exporter`, `dcim_postgres_exporter`, `dcim_redis_exporter`, `dcim_kafka_exporter`, `dcim_elasticsearch_exporter`. Konfigurasi Prometheus + Grafana di `observability/` untuk tim monitoring eksternal (`10.70.0.25`). Commit `baa80ea`.
> - **Metric Mapping Update**: `configs/metric_mapping.json` diperbarui — NAS poller snake_case field names, Vault loader integration. Commit `10a957e`.
> - **Telegram Alerter Timer**: `dcim-telegram-alerter.timer` status **DEAD** (timer tidak men-trigger). Service tersedia tetapi tidak berjalan otomatis.
> - **Project Cleanup**: File-file one-off (`.bak`, debug scripts, scratch files) dipindahkan ke `_archived/`. Struktur project dirapikan.
...
| Timer | Service | Jadwal | Status v4.5.2 |
|---|---|---|---|
...
| `dcim-telegram-alerter.timer` | `dcim-telegram-alerter.service` | Setiap **5 menit** | ⚠️ **DEAD** (timer tidak men-trigger; service tersedia) |

### Systemd — Dinonaktifkan (v4.5.2)
...
### NiFi Process Groups (Data Ingestion) — Live v4.5.2
...
| **Elasticsearch + Kibana** | `elasticsearch/docker-compose.yml` | `dcim_elasticsearch` (9.3.1), `dcim_kibana` (9.3.1) |
| **PostgreSQL** | (external) | `dcim_sot_postgres` (PG 15-alpine) |
| **TimescaleDB** | (external) | `dcim-timescaledb` (PG 15) |
| **iTop** | `itop/docker-compose.yml` | `itop-web`, `itop-db` (MariaDB 10.11), `itop-cloudbeaver` |
| **Ralph** | `ralph/docker-compose.yml` | `ralph_web`, `ralph_nginx`, `ralph_inkpy`, `docker-db-1` (MySQL 8.0), `docker-redis-1` |
| **Prometheus Exporters** | `exporters/docker-compose.yml` | `dcim_node_exporter`, `dcim_postgres_exporter`, `dcim_redis_exporter`, `dcim_kafka_exporter`, `dcim_elasticsearch_exporter` |
| **Observability** | `observability/docker-compose.yml` | Prometheus + Grafana (dikonfigurasi untuk tim monitoring eksternal `10.70.0.25`) |
| **PgAdmin** | (standalone) | `dcim_pgadmin` |

---

*Dokumen ini diperbarui (v4.5.2, 2026-07-24) berdasarkan verifikasi langsung terhadap:*
- *Git history: commit `4804653` (v4.5.1 CCTV NiFi) → `10a957e` (HEAD, metric mapping + NAS + Vault loader)*
...
- *Systemd: 12 services active, 3 timers (1 DEAD: telegram), `dcim-dlq-consumer` auto-restarting*
- *Docker: 25 containers running (termasuk 5 Prometheus exporters + ES 9.3.1 + Kibana 9.3.1)*
...
- *Kafka broker `advertised.listeners`: `10.70.0.56:9094` (SSL external access verified)*
