...
> **Changelog v4.1.0 → v4.3.0** — diverifikasi langsung terhadap sistem aktual `srv-rnd-dcim` (10.70.0.56) dan git history:
>
> **Perubahan Infrastruktur (v4.2 → v4.3, commit `e985d32` tanggal 2026-06-30):**
...
>
> **Perubahan Pipeline (v4.1 → v4.2 → v4.3 → NiFi Cutover):**
...
flowchart LR
    ...
    subgraph L13["🧠 AI Training Data Archive"]
        direction TB
        ARCH_JOB["es_to_pg_archive.py<br>dcim-metrics-archive.timer<br>harian 03:00 WIB"]
        ARCH_TBL[("dcim_metrics_archive<br>PostgreSQL, partisi BULANAN<br>retensi panjang, format long")]
        ARCH_VIEW["v_train_* (6 device_type)<br>materialized view pivot<br>auto-refresh"]
    ...
    subgraph L8["📘 CMDB Automation"]
        direction TB
        INV["server_inventory_collector.py<br>→ Kafka dcim.raw.hardware.server.inventory<br>Daily 01:00 WIB"]
    ...
    ES -. "archive (daily 03:00)" .-> ARCH_JOB --> ARCH_TBL --> ARCH_VIEW --> AI_CONSUMER
    ITOP -. "device relations (JOIN)" .-> AI_CONSUMER

    SRV -. "Redfish Deep Scan (01:00)" .-> INV
    INV -. "Publishes to Kafka" .-> KR_SRV_INV
...
| **L11** | AI / Agent Readiness | Dokumentasi dan *skill* untuk agen AI | ✅ Active | Diselaraskan ke v4.2/v4.3 |
...
| **L14** | Event Lineage Tracking | Pelacakan alur data end-to-end | ✅ Active | **Baru di v4.3** |
| **L15** | Infra Self-Monitoring | Health check ES + PG → Elasticsearch | ✅ Active | **Baru di v4.3** |
| **L16** | Data Quality Assurance | Validasi kelengkapan field per device_type | ✅ Active | **Baru di v4.3** |

---

## 3. Infrastruktur Platform

v4.3 memperkenalkan tiga komponen infrastruktur baru yang mendukung seluruh pipeline.
...
**Perubahan dari v4.1**: Di v4.1, Kafka berjalan sebagai single broker `localhost:9092` tanpa enkripsi. Di v4.3, semua producer dan consumer terhubung via **SSL port `9094`** menggunakan `tls_ca` yang menunjuk ke `kafka/certs/ca-cert.pem`. Inter-broker communication menggunakan listener `INTERNAL` (PLAINTEXT, karena berada dalam Docker network yang sama).
...
**Perubahan dari v4.1**: Di v4.1, semua credential di-hardcode di konfigurasi atau dibaca dari file plain. Di v4.3, `LineageTracker` dan service baru menggunakan Vault sebagai sumber utama credential.
...
### 4.1 Server — Redfish (⚡ Perubahan v4.3)
...
**Penambahan v4.3 — Redfish Telemetry Poller** (`scripts/redfish_telemetry_poller.py`):
...
### 4.2 UPS — NiFi ExecuteProcess (⚡ Perubahan v4.3)
...
### 4.4 Network — NiFi ExecuteProcess (⚡ Perubahan v4.3)
...
### 4.5 CCTV & NVR — Daemon Standalone (⚡ Perubahan v4.3)
...
### 4.6 Telegraf Output — Granular Topic Routing (⚡ Perubahan v4.3)

**Konfigurasi induk**: `configs/telegraf/telegraf_producer.conf`

Di v4.1, Telegraf mengirim semua metrik DCIM ke topik monolitik. Di v4.3, setiap `outputs.kafka` block memiliki `namepass` filter untuk mengarahkan data ke topik spesifik:
...
**Tambahan v4.3 — System Self-Metrics**: Telegraf juga mengumpulkan metrik server DCIM sendiri (`inputs.cpu`, `inputs.disk`, `inputs.mem`, dll.) namun metrics ini **tidak dikirim ke topik DCIM**. Mereka diarahkan ke `dcim-infra-metrics-*` via output Elasticsearch terpisah di `infra-monitoring.conf`.
...
## 8. L6 — Persist (Consumers) (⚡ Perubahan Signifikan v4.3)

Tiga *consumer* independen mengkonsumsi data dari topik Kafka:

### 8.1 `dcim-sql-consumer.service` → PostgreSQL (⚡ Perubahan v4.3)
...
### 8.2 `dcim-es-consumer.service` → Elasticsearch (⚡ **BARU v4.3**)
...
| Aspek | v4.1 (`telegraf-consumer.service`) | v4.3 (`dcim-es-consumer.service`) |
|---|---|---|
...
### 8.3 `dcim-itop-unified.service` (v8) → iTop (⚡ Perubahan v4.3)
...
## 10. L8 — CMDB Automation (⚡ Perubahan v4.3)

### 10.1 `server_inventory_collector.py` — Unified Pipeline Compliant (⚡ Perubahan v4.3)

| Item | v4.1 (`server_inventory_to_pg.py`) | v4.3 (`server_inventory_collector.py`) |
|---|---|---|
...
**Alasan perubahan**: Di v4.1, `server_inventory_to_pg.py` menulis langsung ke PostgreSQL, bypass pipeline Kafka/Normalizer/Enrichment. Di v4.3, inventory data dikirim ke Kafka dan mengikuti alur pipeline standar sehingga:
...
Layer alerting v4.3 terdiri dari **dua jalur independen** yang tetap sama dengan v4.1:
...
**Perubahan v4.3**: DLQ consumer kini mengintegrasikan **lineage tracking** — setiap event yang masuk DLQ di-track melalui `track_lineage()` dengan `status="dlq"`.
...
### 13.1 AI Data Interface (⚡ Baru v4.2/v4.3)
...
> **Status v4.3**: ✅ **Implementasi aktif** — tabel, partisi, materialized views, dan timer sudah dibuat

### 15.1 Perubahan dari v4.1

| Aspek | v4.1 (Proposal) | v4.3 (Aktif) |
|---|---|---|
...
| **Archival Script** | Belum ada | ✅ `scripts/es_to_pg_archive.py` (mode `--backfill` & `--incremental`) |
...
## 16. L14 — Event Lineage Tracking (⚡ Baru v4.3)
...
## 17. L15 — Infrastructure Self-Monitoring (⚡ Baru v4.3)
...
## 18. L16 — Data Quality Assurance (⚡ Baru v4.3)
...
| **CPU/Memory utilization** | ✅ Ready | `redfish_telemetry_poller.py` (v4.3 baru) |
...
| Gap di v4.1 | Status v4.3 | Solusi |
|---|---|---|
...
| Waktu | Skrip | Fungsi | Perubahan v4.3 |
|---|---|---|---|
...
| Service | Skrip | Fungsi | Status v4.3 |
|---|---|---|---|
...
| `dcim-sql-consumer.service` | `event_logger/executor.py` | Avro enriched → PostgreSQL + local enrichment | ✅ Aktif (**rewrite v4.3**) |
...
| Timer | Service | Jadwal | Status v4.3 |
|---|---|---|---|
...
*Dokumen ini diperbarui (v4.3.0, 2026-07-01) berdasarkan verifikasi langsung terhadap:*
