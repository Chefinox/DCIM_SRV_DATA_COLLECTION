...
### 4.5 CCTV & NVR — Apache NiFi ExecuteProcess (⚡ Perubahan v4.4)

| Item | v4.1 | v4.4 |
|---|---|---|
| **Collector** | Telegraf `inputs.exec` (`hikvision_poller.py`, one-shot) | **NiFi ExecuteProcess** (`cctv_poller.py`) |
| **Lifecycle** | Telegraf menjalankan skrip setiap 120s | Dikelola tersentralisasi oleh Apache NiFi scheduler (120s) |
| **Output** | stdout → Telegraf → Kafka | Langsung ke Kafka via `KafkaClient` |
| **Output topic** | `dcim.raw.device.isapi` | `dcim.raw.device.isapi` |

**Alasan perubahan**: Sentralisasi seluruh data ingestion (*polling*) menggunakan Apache NiFi. Rencana awal menggunakan daemon `dcim-cctv-poller.service` dibatalkan dan digantikan oleh NiFi flow agar lebih mudah dikelola.
...
|---|---|---|
| **Storage Engine** | PostgreSQL Partisi Bulanan (`dcim_metrics_archive`) | **TimescaleDB** (Hypertable `dcim_analytics_metrics`) |
...
| `dcim-siem-es-consumer.service` | `siem_es_consumer/app.py` | **BARU** — SIEM alerts → Elasticsearch | ✅ Aktif |
| `dcim-analytics-bridge.service` | `dcim_analytics_bridge.py` | **BARU** — Avro → JSON for Analytics | ✅ Aktif |
| `dcim-analytics-stream-processor.service` | `analytics_stream_processor.py`| **BARU** — Kafka JSON → TimescaleDB | ✅ Aktif |
