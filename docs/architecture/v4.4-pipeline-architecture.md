...
> **Menggantikan**: `docs/architecture/v4.4-pipeline-architecture.md` (2026-07-01, tetap dipertahankan sebagai arsip referensi)
...
> - **Serial Number Resolution**: Enhanced fallback untuk Redfish servers (chassis serial + Lenovo XCC source tag). Lihat §6.2.
...
| **Schema Registry** | Tidak ada | **Confluent SR** `localhost:8081` | **Confluent SR** |
| **Kafka koneksi** | `localhost:9092` PLAINTEXT | `localhost:9094` **SSL** | `localhost:9094` **SSL** |
| **Lineage tracking** | Tidak ada | **`track_lineage()`** dipanggil per event | **`track_lineage()`** |
| **Field computation** | Parsing dasar | memory %, NAS volume %, UPS multi-phase load | **+ computed power metrics** |
| **Metric resolution** | 1 metric per message | 1 metric per message | **N metrics per message** (primary + secondary + computed) |
...
6. **Field computation**:
   - **CCTV/NVR memory%**: Jika `memoryUsage` dan `memoryAvailable` ada → hitung `memoryUsagePct`, `memoryTotal`
...
7. **Resolve metrics** via `resolve_metrics()` dari `metric_mapping.json` → list of `(metric_name, metric_value, metric_unit, severity)` tuples — mencakup primary metric + semua `secondary_metrics` + computed metrics
...
| Energy metrics (`total_facility_power`, `it_equipment_power`) tidak ada | ✅ **Resolved** | Computed metrics di normalizer v4.5 |
...
*Dokumen ini diperbarui (v4.5.0, 2026-07-20) berdasarkan verifikasi langsung terhadap:*
