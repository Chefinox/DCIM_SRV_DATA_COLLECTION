...
### 1. UPS Data Check
```bash
curl -k -s -u elastic:'ES_OLD_PASSWORD_REDACTED' \
...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/visualization/dcim-mon-ups-list'
```
...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/visualization/dcim-mon-devices'
```
...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/dashboard/dcim-monitoring'
```
...
curl -k -X POST 'https://10.70.0.56:9200/dcim-metrics-unified-2026.05.13/_doc' \
  -u elastic:'ES_OLD_PASSWORD_REDACTED' \
  -H 'Content-Type: application/json' \
  -d '{
