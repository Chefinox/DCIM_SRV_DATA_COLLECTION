...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/visualization/dcim-mon-srv-list'
```
...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/visualization/dcim-mon-srv-list' | jq '.attributes.visState' | jq -r '.' | jq '.aggs[] | select(.schema == "bucket") | {field: .params.field, label: .params.customLabel}'
```
