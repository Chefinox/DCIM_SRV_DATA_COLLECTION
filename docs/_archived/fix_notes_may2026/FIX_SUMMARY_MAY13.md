...
```bash
PGPASSWORD='literal:<SET_LOCALLY>' psql -h 192.168.101.73 -U sot_admin -d dcim_sot -c "SELECT COUNT(*) FROM unified_assets;"
```
...
```bash
curl -s -u elastic:'ES_OLD_PASSWORD_REDACTED' 'http://10.70.0.56:5601/api/saved_objects/index-pattern/dcim-metrics-*' | jq '{id, title: .attributes.title, fields: (.attributes.fields | fromjson | length)}'
```
