...
def safe_commit(consumer, offsets_to_commit: dict):
    ...
    try:
        ...
        log.warning(json.dumps({"event": "commit_warning", "error": str(e)}))
    except Exception as e:
        log.error(json.dumps({"event": "commit_error", "error": str(e)}))
    ...
    while True:
        ...
        if len(batch) > 0 and now - last_flush_time > 5.0:
            ...
            if resp.status_code in (200, 201):
                safe_commit(consumer, batch_offsets)
        ...
        try:
            ...
            batch_offsets[key] = max(batch_offsets.get(key, -1), msg.offset())
