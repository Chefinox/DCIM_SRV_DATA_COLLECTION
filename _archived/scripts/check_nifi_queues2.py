...
res = requests.post(f"{BASE_URL}/access/token", data={'username': 'admin', 'password': 'literal:<SET_LOCALLY>'}, verify=False)
...
def search_pg(pg_id):
    ...
    if pg['processGroupFlow']['breadcrumb']['breadcrumb']['name'] == 'Security SIEM Ingestion':
        for conn in pg['processGroupFlow']['flow']['connections']:
            c = conn['component']
            print(f"Connection: {c.get('name', '')} from {c['source'].get('name', '')} to {c['destination'].get('name', '')}")
