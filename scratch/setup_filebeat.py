...
if 'index: "dcim-logs-%{+yyyy.MM.dd}"' not in content:
    # Need to add index to output.elasticsearch
    # Find the output.elasticsearch block
    if 'output.elasticsearch:' in content:
        import re
        content = re.sub(
        ...
        # also need to disable setup.ilm.enabled for custom index
        if 'setup.ilm.enabled' not in content:
            content += "\nsetup.ilm.enabled: false\n"

with open('scratch/new_filebeat.yml', 'w') as f:
    f.write(content)
